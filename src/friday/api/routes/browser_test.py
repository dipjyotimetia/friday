...
async def upload_yaml_scenarios(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to upload YAML scenarios: {error_message}"
        )
...
async def execute_yaml_scenarios(request: YamlScenarioExecuteRequest):
    ...
    try:
        ...
        logger.info(f"Executing {len(test_cases)} scenarios from test suite: {test_suite.name}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to execute YAML scenarios: {error_message}"
        )
...
async def get_yaml_template():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate YAML template: {error_message}"
        )